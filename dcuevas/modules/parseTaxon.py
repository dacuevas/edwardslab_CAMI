...
defaultdir = "/home3/redwards/Cami_Challenge/ncbi-taxonomy_20150130/"
...
class taxonName:
     def __init__(self, t = None, sn = None, bn = None, u = None):
        self.taxid = t
        self.sname = sn
        self.bname = bn
        self.unique = u
...
def readNames():
    ...
    for line in fin:
        line = line.rstrip('\t|\n')
        cols = line.split('\t|\t')
        t = taxonName(cols[0], None, None, cols[2])
        if "scientific name" in cols[3]:
            if cols[0] not in ids:
                t.sname = cols[1]
                ids[cols[0]] = t
            else:
                ids[cols[0]].sname = cols[1]
            #snames[cols[1]] = cols[0]
        if "blast name" in cols[3]:
            if cols[0] not in ids:
                t.bname = cols[1]
                ids[cols[0]] = t
            else:
                ids[cols[0]].bname = cols[1]
            #bnames[cols[1]] = cols[0]

    fin.close()
    return snames, bnames, ids
